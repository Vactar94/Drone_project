...
class Accueil(FloatLayout):
    ...
    def __init__(self, ui_screen, **kwargs):
        ...
        background = RoundedRectangle_hint(color=(0 / 255, 200 / 255, 255 / 255),
                                           pos_hint={"x": 0.025, "center_y": 0.5}, size_hint=(1.08, 0.93),
                                           radius=[30, 30])
        ...
        titre = Updatable_Label(id_text='app.accueil.titre', size_hint_y=None, color=(1, 1, 1), height=200,
                                pos_hint={"center_x": 0.6, "center_y": 0.85}, font_size_type="titre", markup=True)

        # ------------- bouton pour connecter la mannette  -------------#
        conectivity_contoler = Button(text="", size_hint=(None, None), size=(80, 40),
                                      pos_hint={"center_x": 0.095, "center_y": 0.9}, background_color=(0, 0, 0, 0),
                                      background_normal="")
        conectivity_contoler.bind(pos=self.update_bg, size=self.update_bg)
        with conectivity_contoler.canvas.before:
            Color(0, 0, 0)
            conectivity_contoler.bg_rect = Image(source="image/bg_mannette_menue.png", pos=conectivity_contoler.pos,
                                                 size=conectivity_contoler.size)
        ...
        conectivity_drone = Button(text="", size_hint=(None, None), size=(80, 40),
                                   pos_hint={"center_x": 0.095, "center_y": 0.1}, background_color=(0, 0, 0, 0),
                                   background_normal="")
        conectivity_drone.bind(pos=self.update_bg, size=self.update_bg)
        with conectivity_drone.canvas.before:
            Color(0, 0, 0)
            conectivity_drone.bg_rect = Image(source="image/bg_drone_menue.png", pos=conectivity_drone.pos,
                                              size=conectivity_drone.size)
        ...
        button_test = Button(text="go to test", size_hint=(1.0 / 3.0, 1.0 / 10.0),
